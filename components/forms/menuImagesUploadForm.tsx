...
export default function MenuImagesUploadForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (fileInput.files && fileInput.files.length > 0) {
      ...
      try {
        ...
        if (response.ok) {
          toast.success("Menu images uploaded successfully!");
          form.reset();
        } else {
          throw new Error("Failed to upload Menu images");
        }
      } catch (error) {
        toast.error("Failed to upload Menu images. Please try again.");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button type="submit" disabled={isLoading}>
        {isLoading ? "Uploading..." : "Upload Menus Images"}
      </Button>
    </form>
  );
}
